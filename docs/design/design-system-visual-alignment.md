...
**Design system source:** [MASTER.md](../../client/design-system/cookplanner/MASTER.md)  
**Status:** Design  
...
This design aligns the CookPlanner web frontend with the dark analytics palette and typography defined in `client/design-system/cookplanner/MASTER.md`. The implementation is **CSS-only**: global design tokens, Tailwind utility remapping, and accessibility defaults live in a single stylesheet. Component logic, routing, API calls, hooks, and contexts are unchanged except for one anti-pattern fix in `AICookingAssistant`.
