...
export function ShoppingList({ onBack }: ShoppingListProps) {
    ...
    const [selectedIndex, setSelectedIndex] = useState(-1);  // ?°å?ï¼šéµ?¤å??ªç´¢å¼?

    // Shopping item search dropdownï¼ˆç›´?¥ç”¨ nameï¼Œé‰¤å­å…§ debounceï¼?
    const { filteredIngredients: filteredShoppingIngredients, loading: shoppingLoading } = useSearchIngredients(
        newShoppingItem.name,
        ingredients
    );

    // Auto-select if exactly one match and exact name matchï¼ˆç”¨ useMemo ?ªå?ï¼Œé¿?å¾ª?°ï?
